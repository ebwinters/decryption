...
def main_loop1():
	...
	listperm = list(itertools.permutations(alpha, 4))
	perms = Perms.select()
	delte_query = Perms.delete().where(len(str(Perms.key)) > 0)
	while i < len(listperm):
		...
		Perms.create(msg=decrypted_message, key=string_key)
		i += 1
	perms = Perms.select()
	perms = perms.where(Perms.msg.contains("fuck"))
	for perm in perms:
		print (perm.msg)
		print (perm.key)
